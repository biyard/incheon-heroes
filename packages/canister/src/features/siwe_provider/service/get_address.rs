...
use crate::features::siwe_provider::{PRINCIPAL_ADDRESS, SETTINGS};
...
#[query]
pub fn list_addresses() -> Vec<AddressMap> {
    PRINCIPAL_ADDRESS.with(|pa| {
        pa.borrow()
            .iter()
            .map(|(principal, address)| {
                let address = bytes_to_eth_address(&address);
                let principal = Principal::from_slice(principal.as_ref()).to_string();

                AddressMap { principal, address }
            })
            .collect()
    })
}
