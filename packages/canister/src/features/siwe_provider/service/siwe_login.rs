...
use crate::features::siwe_provider::{
    update_root_hash, ADDRESS_PRINCIPAL, PRINCIPAL_ADDRESS, SETTINGS, STATE,
};
...
#[update]
fn siwe_register(signature: String, address: String, timestamp: u64) -> Result<(), String> {
    let principal: Blob<29> = ic_cdk::caller().as_slice()[..29]
        .try_into()
        .map_err(|_| format!("Invalid principal: {:?}", ic_cdk::caller()))?;
    let address = EthAddress::new(&address)?;
    let signature = EthSignature::new(&signature)?;
    let msg = format!("SIWE Register: {}", timestamp);
    let recovered_address = recover_eth_address(&msg, &signature)?;
