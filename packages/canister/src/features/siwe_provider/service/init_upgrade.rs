...
use crate::features::siwe_provider::SETTINGS;
...
pub fn siwe_init(settings_input: SettingsInput) {
    ...
    if let Some(targets) = settings_input.targets {
        let mut targets: Vec<Principal> = targets
            .into_iter()
...
/// settings after users have started using the service!
#[update]
fn init_siwe_provider(settings: SettingsInput) {
    siwe_init(settings);
}
